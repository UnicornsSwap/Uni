...
contract UniToken is ERC20("Unicorns.Finance", "UNI"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the UniPool.
...
contract UniPool is Ownable {
    ...
    // Bonus muliplier for early uni stakers.
    ...
    constructor(
        ...
        uni.approve(router, 0);
        uni.approve(router, uint(-1));
    }
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        if (block.number > startBlock){
            return uniPerBlock/( 2**( ( blockNumber.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        if (block.number > startBlock){
            return uniPerBlock/( 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function pendingUni(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 UniReward = multiplier.mul(uniPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 halvFactor = 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) );
        uint256 UniReward = multiplier.mul(uniPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
        uint256 UniFunds = UniReward.div(20); // 5%
        uni.mint(address(this), UniReward);
        uni.mint(address(this), UniFunds);
        address[] memory path = new address[](2);
        path[0] = address(uni);
        path[1] = wbnb;
        Swap(router).swapExactTokensForETH(UniFunds, uint256(0), path, funds, now.add(1800));
        pool.accUniPerShare = pool.accUniPerShare.add(UniReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to StakingContract for UNI allocation.
